...
(function(ice, $) {
    ...
    function isIgnoredKeyCode(key) {
        if (verboseLogging) console.trace("isIgnoredKeyCode: key: " + key);
        if (!key) {
            return true;
        }
        var i, len = ignoreKeyCodes.length,
            rec;
    ...
    InlineChangeEditor = function(options) {
        ...
        if (verboseLogging) console.trace("InlineChangeEditor: options: " + options);
    ...
    InlineChangeEditor.prototype = {
        ...
        listenToEvents: function() {
            if (this.element && !this._boundEventHandler) {
                ...
                if (verboseLogging) console.trace("listenToEvents: this.element: " + this.element);
        ...
        unlistenToEvents: function() {
            if (this.element && this._boundEventHandler) {
                if (verboseLogging) console.trace("unlistenToEvents: this.element: " + this.element);
        ...
        _createIceNode: function(changeType, childNode, changeId) {
            if (verboseLogging) console.trace("_createIceNode: changeType: " + changeType);
        ...
        insert: function(options) {
            if (verboseLogging) console.trace("insert: options: " + options);
            ...
            console.info("exiting ice insert method");
            console.info(ret);
        ...
        _deleteContents: function(right, range) {
            if (verboseLogging) console.trace("_deleteContents: range: " + range);
        ...
        acceptRejectChange: function(node, options) {
            ...
            var delSel, insSel, selector, removeSel, replaceSel,
                trackNode, changes, dom = ice.dom,
                nChanges,
                removeChange = options.removeChange !== false,
                self = this,
                changeId, content, userStyle,
                $element = $(this.element),
        ...
        _addChange: function(ctnType, ctNodes, changeIdToUse) {
            if (verboseLogging) console.trace("_addChange: range: " + ctnType);
        ...
        _addNodeToChange: function(changeid, ctNode) {
            if (verboseLogging) console.trace("_addNodeToChange: ctNode: " + ctNode);
        ...
        _insertNodes: function(_range, hostRange, _data) {
            if (verboseLogging) console.trace("_insertNodes: _range: " + _range);
        ...
        prepareToCut: function() {
            var range = this.getCurrentRange(),
                ...
                /// HERE
                listTestRange = this.getCurrentRange();

            var name = listTestRange.commonAncestorContainer.nodeName;
            if (name === "UL" || name === "OL") {
                for (var i = 0; i < listTestRange.commonAncestorContainer.children.length - 1; i += 1) {
                    listTestRange.setStartBefore(listTestRange.startContainer);
                    listTestRange.setEndAfter(listTestRange.endContainer);
                }
                var startname = listTestRange.startContainer.nodeName;
                var endname = listTestRange.endContainer.nodeName;
                if ((startname === "UL" || startname === "OL") && (endname === "UL" || endname === "OL")) {
                    range = listTestRange;
                }
            }
