...
(function(CKEDITOR, global) {
    ...
    function isLITENode(node) {
        if (verboseLogging) console.trace("isLITENode: node: " + node);
        return node && node.className && IS_LITE_CLASS_RE.test(node.className);
    }


    function cleanNode(node) {
        if (verboseLogging) console.trace("cleanNode: node: " + node);
        var ret, name,
            i, child;
    ...
    function cleanClipboard(nodes) {
        if (verboseLogging) console.trace("cleanClipboard");
    ...
    function isCutKeystroke(code) {
        if (verboseLogging) console.trace("isCutKeystroke: code: " + code);
        return cutKeystrokes.indexOf(code) >= 0;
    }

    function nodeToDOMNode(node) {
        if (verboseLogging) console.trace("nodeToDOMNode");
    ...
    function _findPluginIndex(editor) {
        if (verboseLogging) console.trace("_findPluginIndex");
    ...
    function padString(s, length, padWith, bSuffix) {
        if (verboseLogging) console.trace("padString: s, length, padWith, bSuffix: " + s + ", " + length + ", " + padWith + ", " + bSuffix);
    ...
    function padNumber(s, length) {
        if (verboseLogging) console.trace("padNumber");
        return padString(s, length, '0');
    }

    function relativeDateFormat(date, lang) {
        if (verboseLogging) console.trace("relativeDateFormat");
        var now = new Date(),
            today = now.getDate(),
            month = now.getMonth(),
    ...
    function elementMatchesSelectors($el, patterns) {
        if (verboseLogging) console.trace("elementMatchesSelectors: $el: " + $el + " patterns: " + patterns);
        ...
        init: function(ed) {
            ...
            if (required.length) {
                function loadRequired(rec) {
                    if (verboseLogging) console.trace("loadRequired: rec: " + rec + " required.length: " + required.length);
    ...
    LITEPlugin.prototype = {
        ...
        init: function(ed, config) {
            ...
            function add1(rec) {
                if (verboseLogging) console.trace("add1: rec: " + rec);
        ...
        _onKeyDown: function(evt) {
             ...
             */
            console.log("LITE: _onKeyDown with code: " + code);
            console.log("LITE: isCutKeystroke returns: " + isCutKeystroke(code));
            if (isCutKeystroke(code)) {
        ...
        _onPaste: function(evt) {
             ...
             */
            console.log("Track Changes _onPaste: evt: " + evt);
        ...
        _notifyChange: function() {
            if (verboseLogging) console.trace("_notifyChange");
            this._changeTimeout = null;
        ...
        _notifyTextChange: function() {
            if (verboseLogging) console.trace("_notifyTextChange");
            this._changeTimeout = null;
        ...
        _beforeEdit: function() {
            console.log("Track changes _beforeEdit");
        ...
        _hostCopy: function() {
            if (verboseLogging) console.trace("_hostCopy");
            try {
                if (CKEDITOR.env.ie) {
        ...
        _getHostRange: function() {
            if (verboseLogging) console.trace("_getHostRange");
            var selection = this._editor && this._editor.getSelection(),
        ...
        _getHostRangeData: function(hostRange) {
            if (verboseLogging) console.trace("_getHostRangeData");
        ...
        _beforeInsert: function() {
            console.log("Track change _beforeInsert");
            this._editor.fire('lite_beforeInsert');
            this._editor.fire('saveSnapshot');
        },
        ...
        _afterInsert: function() {
            console.log("Track changes _afterInsert");
