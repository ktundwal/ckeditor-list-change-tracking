...
(function() {
    ...
    // removeList helpers
    let selectedListItemsToArray = function (groupObj, database) {
        let removedListItem = [];
        for (var i = 0; i < groupObj.contents.length; i++) {
            var itemNode = groupObj.contents[i]
            itemNode = itemNode.getAscendant('li', true)
            if (!itemNode || itemNode.getCustomData('list_item_processed'))
                continue
            removedListItem.push(itemNode)
            CKEDITOR.dom.element.setMarker(database, itemNode, 'list_item_processed', true)
        }
        return removedListItem;
    }

    let unindent = function (listArray, selectedListItems) {
    ...
    let changeTrackRemoveList = function (listArray, selectedListItems) {
        var lastListIndex = null;
        for (i = 0; i < selectedListItems.length; i++) {
            var listIndex = selectedListItems[i].getCustomData('listarray_index')
            //listArray[listIndex].indent = -1
            listArray[listIndex].data( 'c3-user-intent', 'remove-list' );
            lastListIndex = listIndex
        }
        return lastListIndex;
    }
    ...
    let renumberItemsAfterSplit = function (listArray, lastListIndex) {
    ...
    function removeList(editor, groupObj, database) {
        ...
        var listArray = CKEDITOR.plugins.list.listToArray(groupObj.root, database),
            selectedListItems = selectedListItemsToArray(groupObj, database);

        var lastListIndex = unindent(listArray, selectedListItems);
        renumberItemsAfterSplit(listArray, lastListIndex)

        var newList = CKEDITOR.plugins.list.arrayToList(listArray, database, null,
            editor.config.enterMode, groupObj.root.getAttribute('dir'));
        ...
        docFragment = wrapEntireListInIns(docFragment);
    ...
    // this method was written to explore an alternative where we would mark the list as modified but not modify it.
    function markRemovedListItems(editor, groupObj, database) {

        for (var i = 0; i < groupObj.contents.length; i++) {
            var itemNode = groupObj.contents[i]
            itemNode.data('c3-user-intent', 'remove-list');
        }
    ...
    // do following
    // <ins class="ice-ins ice-cts" data-cid="2" data-userid="" data-username=""
    // data-changedata="" data-time="1522442297531"
    // data-last-change-time="1522442297531">f</ins>
    function wrapInIns(html) {
        var currentTime = (new Date()).getTime();
        var changeId = 2;
        return `<ins class="ice-ins ice-cts" data-cid="${changeId}" 
                data-userid="" data-username="" data-changedata="" 
                data-time="${currentTime}" data-last-change-time="${currentTime}">${html}</ins>`;
    }

    function wrapEntireListInIns(fragment) {
        for (var i = 0; i < fragment.$.childNodes.length; i++) {
            var childNode = fragment.$.childNodes[i];
            childNode.innerHTML = wrapInIns(childNode.innerHTML);
        }
        return fragment;
    }
